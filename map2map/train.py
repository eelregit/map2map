...
def gpu_worker(local_rank, node, args):
    ...
    if args.div_data:
        train_sampler = GroupedRandomSampler(
            train_dataset,
            group_size=None
            if args.cache_maxsize is None
            else args.cache_maxsize * train_dataset.ncrop,
        )
    else:
        try:
    ...
    if args.adv:
        ...
        adv_model = adv_model(
            sum(args.in_chan + args.out_chan) if args.cgan else sum(args.out_chan), 1
        )
        if args.adv_model_spectral_norm:
            add_spectral_norm(adv_model)
        adv_model.to(device)
        adv_model = DistributedDataParallel(
            adv_model, device_ids=[device], process_group=dist.new_group()
        )
        ...
        adv_scheduler = optim.lr_scheduler.ReduceLROnPlateau(
            adv_optimizer, factor=0.1, patience=10, verbose=True
        )
    ...
    if args.adv:
        args.instance_noise = InstanceNoise(
            args.instance_noise, args.instance_noise_batches
        )
    ...
    if args.cache:
        print(
            "rank {} train data: {}".format(rank, train_dataset.get_fields.cache_info())
        )
        print(
            "rank {} val   data: {}".format(rank, val_dataset.get_fields.cache_info())
        )
...
def train(
    ...
    for i, (input, target) in enumerate(loader):
        ...
        if hasattr(model, "scale_factor") and model.scale_factor != 1:
            input = F.interpolate(
                input, scale_factor=model.scale_factor, mode="nearest"
            )
        ...
        if batch % args.log_interval == 0:
            ...
            if rank == 0:
                ...
                if args.adv and epoch >= args.adv_start:
                    logger.add_scalar(
                        "loss/batch/train/adv/G", loss_adv.item(), global_step=batch
                    )
                    logger.add_scalars(
                ...
                grads = list(
                    p.grad for n, p in model.named_parameters() if ".weight" in n
                )
                grads = [grads[0], grads[-1]]
                grads = [g.detach().norm().item() for g in grads]
                logger.add_scalars(
                    "grad", {"first": grads[0], "last": grads[-1],}, global_step=batch
                )
                if args.adv and epoch >= args.adv_start:
                    grads = list(
                        p.grad
                        for n, p in adv_model.named_parameters()
                        if ".weight" in n
                    )
                    grads = [grads[0], grads[-1]]
                    grads = [g.detach().norm().item() for g in grads]
    ...
    if rank == 0:
        ...
        if args.adv and epoch >= args.adv_start:
            logger.add_scalar(
                "loss/epoch/train/adv/G", epoch_loss[1], global_step=epoch + 1
            )
            logger.add_scalars(
                "loss/epoch/train/adv/D",
                {"total": epoch_loss[2], "fake": epoch_loss[3], "real": epoch_loss[4],},
                global_step=epoch + 1,
            )
...
def validate(
    epoch, loader, model, criterion, adv_model, adv_criterion, logger, device, args
):
    model.eval()
    if args.adv:
        adv_model.eval()
    ...
    if rank == 0:
        ...
        if args.adv and epoch >= args.adv_start:
            logger.add_scalar(
                "loss/epoch/val/adv/G", epoch_loss[1], global_step=epoch + 1
            )
            logger.add_scalars(
