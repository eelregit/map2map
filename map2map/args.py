...
def add_common_args(parser):
    parser.add_argument(
        "--in-norms",
        type=str_list,
        help="comma-sep. list " "of input normalization functions from .data.norms",
    )
    parser.add_argument(
        "--tgt-norms",
        type=str_list,
        help="comma-sep. list " "of target normalization functions from .data.norms",
    )
    parser.add_argument(
        "--crop", type=int, help="size to crop the input and target data"
    )
    parser.add_argument(
        "--pad",
        default=0,
    ...
    parser.add_argument("--model", required=True, type=str, help="model from .models")
    parser.add_argument(
        "--criterion", default="MSELoss", type=str, help="model criterion from torch.nn"
    )
    parser.add_argument(
        "--load-state",
        default=ckpt_link,
...
def add_train_args(parser):
    ...
    parser.add_argument(
        "--adv-model", type=str, help="enable adversary model from .models"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--optimizer", default="Adam", type=str, help="optimizer from torch.optim"
    )
    parser.add_argument("--lr", default=0.001, type=float, help="initial learning rate")
    #    parser.add_argument('--momentum', default=0.9, type=float,
    #            help='momentum')
    parser.add_argument("--weight-decay", default=0, type=float, help="weight decay")
    parser.add_argument("--adv-lr", type=float, help="initial adversary learning rate")
    parser.add_argument("--adv-weight-decay", type=float, help="adversary weight decay")
    parser.add_argument(
...
def set_common_args(args):
    ...
    if not args.cache and args.cache_maxsize is not None:
        ...
        warnings.warn("Resetting cache maxsize given cache is disabled", RuntimeWarning)
    if args.cache and args.cache_maxsize is not None and args.cache_maxsize < 1:
        args.cache = False
        args.cache_maxsize = None
        warnings.warn("Disabling cache given cache maxsize < 1", RuntimeWarning)
